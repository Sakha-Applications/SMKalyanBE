// File: transform_father_occupation.js
 ...
 */
function parseFatherOccupation(rawOccupationStr) {
    ...
    // 1. Handle special statuses / non-professions first
    if (cleanedStr.includes('retired') || cleanedStr.includes('retd') || cleanedStr.includes('late') ||
        ...
        return 'Retired'; // Using 'Retired' as a common standardized status, or 'Not Applicable' if preferred
    }
    if (cleanedStr.includes('student') || cleanedStr.includes('studying')) {
        return 'Student'; // New category if needed
    }
    if (cleanedStr.includes('purohit') || cleanedStr.includes('archaka') || cleanedStr.includes('panditharu') || cleanedStr.includes('priest')) {
        return 'Teacher'; // Mapping Purohit/Archaka to Teacher as per your canonical list if more specific is not desired
    }
    if (cleanedStr.includes('businessperson') || cleanedStr.includes('business man') || cleanedStr.includes('entrepreneur') || cleanedStr.includes('owner') || cleanedStr.includes('proprietor')) {
        return 'Businessperson';
    }
    if (cleanedStr.includes('homemaker')) {
        return 'Homemaker';
    }

    // 2. Iterate through canonical professions (longest first) to find a match
    ...
    // 3. Fallback heuristics if no direct match
    ...
    if (cleanedStr.includes('accountant') || cleanedStr.includes('audit') || cleanedStr.includes('finance') || cleanedStr.includes('cfo')) return 'Accountant'; // Broaden to Accountant
    ...
    if (cleanedStr.includes('manager') || cleanedStr.includes('executive') || cleanedStr.includes('officer') || cleanedStr.includes('director') || cleanedStr.includes('head')) return 'Businessperson'; // General management/executive roles
    ...
    if (cleanedStr.includes('politician') || cleanedStr.includes('govnt') || cleanedStr.includes('government')) return 'Politician'; // Assuming government worker maps to Politician for broadness
    if (cleanedStr.includes('psychologist')) return 'Psychologist';
    if (cleanedStr.includes('farmer') || cleanedStr.includes('agriculture')) return 'Businessperson'; // Mapping Farmer to Businessperson for broadness
    if (cleanedStr.includes('police')) return 'Government'; // New category or map to Politician/Other? Let's map to Politician for broadness for now.

    // Catch-all if nothing specific matched but it's not a known null/retired status
    if (cleanedStr.length > 2) { // Avoid mapping very short remnants
...
async function transformFatherOccupationData() {
    let connection;
    try {
        ...
        // 1. Fetch all profile_id_raw and father_occupation_raw
        const [profiles] = await connection.query(`SELECT profile_id_raw, father_occupation_raw FROM profiles_staging`);
        console.log(`Fetched ${profiles.length} profiles for father's occupation transformation.`);

        const updates = [];
        for (const profile of profiles) {
            const originalOccupation = profile.father_occupation_raw;
            const standardizedOccupation = parseFatherOccupation(originalOccupation);
        ...
        for (let i = 0; i < updates.length; i += batchSize) {
            ...
            let updateQueries = batch.map(item => {
                ...
                return `UPDATE profiles_staging SET father_occupation_raw = ${occupationValue} WHERE profile_id_raw = ${connection.escape(item.profile_id_raw)};`;
            }).join('\n');
        ...
        console.log('Father\'s occupation transformation completed successfully.');

    } catch (error) {
        console.error('Error during father\'s occupation transformation:', error.message);
        if (error.sqlMessage) {
